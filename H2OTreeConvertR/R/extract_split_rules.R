...
#'
#' @export
get_terminal_node_directions <- function(df, node, current_str = NULL, unique_expr = TRUE) {
  ...
  if (is.na(df[df$node == node, 'left_split'])) {
    ...
    if (!is.na(df[df$node == node, 'NA_direction'])) {
      ...
      if (df[df$node == node, 'NA_direction'] == 'left') {
        ...
        left_current_str[length(left_current_str)] <- paste0('(',
        ...
        right_current_str[length(right_current_str)] <- paste0('(',
    ...
    l <- get_terminal_node_directions(df, df[df$node == node, 'left_split'], left_current_str)

    # call function again on the right child node
    r <- get_terminal_node_directions(df, df[df$node == node, 'right_split'], right_current_str)
...
#' Get split conditions for terminal nodes.
#'
#' This is a wrapper for get_terminal_node_directions to do input checking and return
...
terminal_node_exprs <- function(split_exprs) {
  ...
  # this will be a concatenation of each node visited to get to the terminal node
  #   separated by '.'
  ...
  terminal_node_path_split <- sapply(terminal_node_path_split,
                                     function(x) c(top_node, x[1:(length(x)-1)]))
